...
Singleton {
    ...
    property string materialSymbol: root.ethernet ? "lan"
    ...
    function enableWifi(enabled = true) { enableWifiProc.exec(["nmcli", "radio", "wifi", enabled ? "on" : "off"]); }
    function toggleWifi() { enableWifi(!wifiEnabled); }
    function rescanWifi() { wifiScanning = true; rescanProcess.running = true; }
    ...
    Process {
        ...
        stderr: SplitParser { onRead: line => { if (line.includes("Secrets were required")) root.wifiConnectTarget.askingPassword = true; } }
        onExited: (exitCode, exitStatus) => { root.wifiConnectTarget.askingPassword = (exitCode !== 0); root.wifiConnectTarget = null; }
    }
    Process { id: rescanProcess; command: ["nmcli", "dev", "wifi", "list", "--rescan", "yes"]; stdout: SplitParser { onRead: { wifiScanning = false; getNetworks.running = true; } } }

    function update() { updateConnectionType.startCheck(); wifiStatusProcess.running = true; updateNetworkName.running = true; updateNetworkStrength.running = true; }
    Process { id: subscriber; running: true; command: ["nmcli", "monitor"]; stdout: SplitParser { onRead: root.update() } }
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            ...
            let hasEthernet = false; let hasWifi = false; let wifiStatus = "disconnected";
            lines.forEach(line => {
                if (line.includes("ethernet") && line.includes("connected")) hasEthernet = true;
                    ...
                    else if (line.includes("connected")) { hasWifi = true; wifiStatus = "connected"; if (connectivity === "limited") { hasWifi = false; wifiStatus = "limited"; } }
                    else if (line.includes("connecting")) wifiStatus = "connecting";
                    else if (line.includes("unavailable")) wifiStatus = "disabled";
                }
            });
            root.wifiStatus = wifiStatus; root.ethernet = hasEthernet; root.wifi = hasWifi;
        }
    }
    Process { id: updateNetworkName; command: ["sh", "-c", "nmcli -t -f NAME c show --active | head -1"]; running: true; stdout: SplitParser { onRead: data => { root.networkName = data; } } }
    Process { id: updateNetworkStrength; running: true; command: ["sh", "-c", "nmcli -f IN-USE,SIGNAL,SSID device wifi | awk '/^\\*/{if (NR!=1) {print $2}}'"]; stdout: SplitParser { onRead: data => { root.networkStrength = parseInt(data); } } }
    Process { id: wifiStatusProcess; command: ["nmcli", "radio", "wifi"]; Component.onCompleted: running = true; environment: ({ LANG: "C", LC_ALL: "C" }); stdout: StdioCollector { onStreamFinished: { root.wifiEnabled = text.trim() === "enabled"; } } }
    ...
    function _dedupNetworks(networks) {
        const networkMap = new Map();
    ...
    function _iconForStrength(strength) {
