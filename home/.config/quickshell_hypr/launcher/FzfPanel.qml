...
Rectangle {
    ...
    function acceptCurrent() {
        ...
        // Legacy: clipboard (cliphist needs special handling)
        let selectAction = ai ? (ai.selectAction || "") : "";
        if (selectAction === "clipboard") {
            GlobalStates.fzfPanelOpen = false;
            let cmd = "printf '%b' " + JSON.stringify(label) + " | cliphist decode | wl-copy";
