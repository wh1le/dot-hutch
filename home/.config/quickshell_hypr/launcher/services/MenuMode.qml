...
Singleton {
    ...
    Connections {
        ...
        function onMenuItemsChanged() { root.loadItems(); }
    }
    ...
    function launch(item) {
        ...
        var cmd;
        if (item.option) {
            cmd = "$HOME/.config/fzm/options/" + item.option;
            if (item.args)
                cmd += " " + item.args;
        } else if (item.script) {
            cmd = item.script;
        ...
        // Set per-launch hyprland windowrule for size, then spawn terminal
        var sizeRule = "match:class ^(terminal-quickshell)$, size " + item.width + " " + item.height;
        Quickshell.execDetached({ command: ["hyprctl", "keyword", "windowrule", sizeRule] });
