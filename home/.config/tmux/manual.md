...
command [flags]
This specifies one of a set of commands used to control tmux, as described in the following sections. If no commands are specified, the command in default-client-command is assumed, which defaults to new-session.
...
Move floating pane to corners or edges. Meta resizes the pane as well.
~
...
As in these examples, when running tmux from the shell extra care must be taken to properly quote semicolons:
    ...
    Semicolons that should be interpreted as a command separator should be escaped according to the shell conventions. For sh(1) this typically means quoted (such as ‘neww ';' splitw’) or escaped (such as ‘neww \; splitw’).
    Individual semicolons or trailing semicolons that should be interpreted as arguments should be escaped twice: once according to the shell conventions and a second time for tmux; for example:
...
Command arguments may be specified as strings surrounded by single (') quotes or double quotes ("), or as command lists surrounded by braces ({}). This is required when the argument contains any special character. Single and double quoted strings cannot span multiple lines except with line continuation. Braces can span multiple lines.
...
target-session, target-window or target-pane each denote the ‘type’ of target the command needs to work on. If a target is not explicitly qualified using ‘:’ and ‘.’, tmux will pick what seems to be the best choice available. For example, if a target-pane of ‘1’ is used with a current window that has only one pane, tmux knows that the ‘1’ cannot mean a pane, so it will look for the active pane in window 1, or the active pane in the current window in session 1. Only if none of these are present will it report an error. If it is important that a specific pane, window or session always be used, such as from a script, the target should be fully qualified (for example a target-pane of ‘-t:.1’), or pane, window or session IDs should be used.
...
Destroy the given session, closing any windows linked to it and no other sessions, and detaching all clients attached to it. If -a is given, all sessions but the specified one is killed. When -a is given, -f specifies a filter. Only sessions for which the filter is true are killed. If -g is given and the session is in a session group, all sessions in the group are killed. The -C flag clears alerts (bell, activity, or silence) in all windows linked to the session.
list-clients [-r] [-F format] [-f filter] [-O sort-order] [-t target-session]
(alias: lsc)
List all clients attached to the server. -F specifies the format of each line and -f a filter. Only clients for which the filter is true are shown. See the FORMATS section. -O specifies the sort order: one of ‘name’, ‘size’, ‘creation’ (time), or ‘activity’ (time). -r reverses the sort order. If target-session is specified, list only clients connected to that session.
...
(alias: ls)
List all sessions managed by the server. -F specifies the format of each line and -f a filter. Only sessions for which the filter is true are shown. See the FORMATS section. -O specifies the sort order: one of ‘index’, ‘name’, ‘creation’ (time), or ‘activity’ (time). -r reverses the sort order.
...
refresh-client [-cDlLRSU] [-A pane:state] [-B name:what:format] [-C size] [-f flags] [-r pane:report] [-t target-client] [adjustment]
(alias: refresh)
...
server-access [-adglrw] [user | group]
Change the access or read/write permission of user or group. The user running the tmux server (its owner) and the root user cannot be changed and are always permitted access. -g changes a group rather than a user.

    -a and -d are used to give or revoke access for the specified user or group. If a client is already attached, the -d flag causes it to be detached unless it is still permitted by another entry.

    -r and -w change the permissions for user or group: -r makes matching clients read-only and -w writable. -l lists current access permissions. User entries are shown with ‘U’, group entries with ‘G’, and read-only or writable entries with ‘R’ or ‘W’, for example ‘user1 (U,W)’ or ‘testgroup (G,R)’. If both a user and group entry match a client, the user entry takes precedence. Only the effective group ID of the client is used, not its supplementary groups.
...
switch-client [-ElnprZ] [-c target-client] [-O sort-order] [-t target-session] [-T key-table]
(alias: switchc)
Switch the current session for client target-client to target-session. As a special case, -t may refer to a pane (a target that contains ‘:’, ‘.’ or ‘%’), to change session, window and pane. In that case, -Z keeps the window zoomed if it was zoomed. If -l, -n or -p is used, the client is moved to the last, next or previous session respectively. -O may be used with -n and -p to specify the field to sort on: one of ‘name’, ‘size’, ‘creation’ (time), or ‘activity’ (time). -r toggles the client read-only and ignore-size flags (see the attach-session command).

    If -E is used, update-environment option will not be applied.

    -T sets the client's key table; the next key will be looked up using key-table. After that key, the client is returned to its default key table (normally root). This may be used to configure multiple prefix keys, or to bind commands to sequences of keys. For example, to make typing ‘abc’ run the list-keys command:
...
recentre-top-bottom (emacs : C-l)
Cycles the current line between centre, top, and bottom.
refresh-on
Turn on automatic refresh of the content from the pane, so that new output appears while in copy mode. Automatic refresh is off by default; it will scroll only while the cursor is at the bottom and is paused while a selection is in progress.
refresh-off
Turn off automatic refresh of the content from the pane.
refresh-toggle (vi: r) (emacs: r)
...
Scroll down. If mode-keys is vi, the cursor is fixed relative to the text.
scroll-down-and-cancel
Same as scroll-down but also exit copy mode if the cursor reaches the bottom.
scroll-exit-on
Turn on exiting copy mode when scrolling to the end of the buffer, unless a selection is present.
scroll-exit-off
...
Scroll up. If mode-keys is vi, the cursor is fixed relative to the text.
...
The default incremental search key bindings, ‘C-r’ and ‘C-s’, are designed to emulate emacs(1). When first pressed they allow a new search term to be entered; if pressed with an empty search term they repeat the previously used search term.

The ‘next-prompt’ and ‘previous-prompt’ move between shell prompts, but require the shell to emit an escape sequence (\033]133;A\033\\) to tell tmux where the prompts are located; if the shell does not do this, these commands will do nothing. The -o flag jumps to the beginning of the command output instead of the shell prompt. Finding the beginning of command output requires the shell to emit an escape sequence (\033]133;C\033\\) to tell tmux where the output begins. If the shell does not send these escape sequences, these commands do nothing.

Copy commands may take an optional buffer prefix argument which is used to generate the buffer name (the default is ‘buffer’ so buffers are named ‘buffer0’, ‘buffer1’ and so on). Pipe commands take a command argument which is the command to which the selected text is piped. ‘copy-pipe’ variants also copy the selection. The ‘-and-cancel’ variants of some commands exit copy mode after they have completed (for copy commands) or when the cursor reaches the bottom (for scrolling commands). ‘-no-clear’ variants do not clear the selection. All the copy commands can take the -C and -P flags. The -C flag suppresses setting the terminal clipboard when copying, while the -P flag suppresses adding a paste buffer with the text.
...
Enter copy mode.
    ...
    -M begins a mouse drag (only valid if bound to a mouse key binding, see MOUSE SUPPORT).
    ...
    -e specifies that scrolling to the bottom of the history (to the visible screen) should exit copy mode. This will not happen if a selection is present. While in copy mode, pressing a key other than those used for scrolling will disable this behaviour. This is intended to allow fast scrolling through a pane's history, for example with:

    bind PageUp copy-mode -eu
    bind PageDown copy-mode -ed

    -k kills the pane when the mode is exited.
...
break-pane [-abdPW] [-F format] [-n window-name] [-s src-pane] [-t dst-window] [-x width] [-y height] [-X x-position] [-Y y-position]
(alias: breakp)
Break src-pane off from its containing window to make it the only pane in dst-window. With -a or -b, the window is moved to the next index after or before (existing windows are moved if necessary). If -d is given, the new window does not become the current window. The -P option prints information about the new window after it has been created. By default, it uses the format ‘#{session_name}:#{window_index}.#{pane_index}’ but a different format may be specified with -F.
    ...
    If the -W option is given, src-pane is lifted out of the tiled layout and made floating. The -x and -y options set the width and height of the floating pane in columns and lines respectively. The default is half the window width and a quarter the window height. The -X and -Y options set the position of the upper-left corner of the pane. If omitted, new floating panes are cascaded from the top-left of the window. The -x, -y, -X, and -Y options may be followed by ‘%’ to specify a percentage of the window size.

    If the pane had previously been floating, the position and sizes are restored from the saved values not specified by the -x, -y, -X, and -Y options.

    If -d is given, the active pane is not changed. The pane must not already be floating or hidden, and the window must not be zoomed.

capture-pane [-aeFHLpPRqCJMN] [-b buffer-name] [-E end-line] [-S start-line] [-t target-pane]
(alias: capturep)
Capture the contents of a pane. If -p is given, the output goes to stdout, otherwise to the buffer specified with -b or a new buffer if omitted. If -a is given, the alternate screen is used, and the history is not accessible. If no alternate screen exists, an error will be returned unless -q is given. Similarly, if the pane is in a mode, -M uses the screen for the mode. If -e is given, the output includes escape sequences for text and background attributes. -C also escapes non-printable characters as octal \xxx. -T ignores trailing positions that do not contain a character. -N preserves trailing spaces at each line's end and -J preserves trailing spaces and joins any wrapped lines; -J implies -T. -P captures only any output that the pane has received that is the beginning of an as-yet incomplete escape sequence. -L includes the line number at the start of each line and -F includes the flags (where ‘-’ is no flags, ‘D’ is an unused line, ‘O’ is a line marked as output, ‘P’ is a line marked as a prompt, ‘X’ is a line containing extended cells and ‘H’ is a line with hyperlinks). With -H, only hyperlinks in the specified lines are captured. Multiple hyperlinks on the same line are separated by spaces. -R dumps the internal grid data for diagnostics.
...
v Toggle preview
i Change view (preview and client information)
F1 or C-h Display help
q Exit mode
    ...
    -O specifies the initial sort order: one of ‘name’, ‘size’, ‘creation’ (time), or ‘activity’ (time). -r reverses the sort order. -f specifies an initial filter: the filter is a format - if it evaluates to zero, the item in the list is not shown, otherwise it is shown. If a filter would lead to an empty list, it is ignored. -F specifies the format for each item in the list and -K a format for each shortcut key; both are evaluated once for each line.

    -N starts without the preview or if given twice with the larger preview. -h hides the pane containing the mode. -k kills the pane when the mode is exited.

    The choose-client command works only if at least one client is attached.

choose-tree [-GhikNrswyZ] [-F format] [-f filter] [-K key-format] [-O sort-order] [-t target-pane] [template]
Put a pane into tree mode, where a session, window or pane may be chosen interactively from a tree. Each session, window or pane is shown on one line. A shortcut key is shown on the left in brackets allowing for immediate choice, or the tree may be navigated and an item chosen or otherwise manipulated using the keys below. -s starts with sessions collapsed and -w with windows collapsed. -Z zooms the pane. -y disables any confirmation prompts. The following keys may be used in tree mode:
Key Function
Enter Choose selected item
Up Select previous item
Down Select next item
S-Up Swap the current window with the previous one
S-Down Swap the current window with the next one + Expand selected item - Collapse selected item
M-+ Expand all items
M-- Collapse all items
x Kill selected item
X Kill tagged items
< Scroll list of previews left > Scroll list of previews right
C-s Search by name
...
r Reverse sort order
v Toggle preview
i Change view (preview and session, window and pane information)
F1 or C-h Display help
q Exit mode
    ...
    -O specifies the initial sort order: one of ‘index’, ‘name’, ‘activity’ (time) or ‘z’. -r reverses the sort order. -f specifies an initial filter: the filter is a format - if it evaluates to zero, the item in the list is not shown, otherwise it is shown. If a filter would lead to an empty list, it is ignored. -F specifies the format for each item in the tree and -K a format for each shortcut key; both are evaluated once for each line. -G includes all sessions in any session groups in the tree rather than only the first.

    -N starts without the preview or if given twice with the larger preview. -i starts showing client information instead of the preview. -h hides the pane containing the mode. -k kills the pane when the mode is exited. -h and -k are intended to ease use of the mode in a floating pane; for example:

    bind s 'new-pane -x50% -y50% -E; choose-tree -hks'

    The choose-tree command works only if at least one client is attached.

customize-mode [-kNZ] [-F format] [-f filter] [-t target-pane] [template]
...
Up Select previous item
Down Select next item + Expand selected item - Collapse selected item
M-+ Expand all items
M-- Collapse all items
...
q Exit mode
    ...
    -f specifies an initial filter: the filter is a format - if it evaluates to zero, the item in the list is not shown, otherwise it is shown. If a filter would lead to an empty list, it is ignored. -F specifies the format for each item in the tree.

    -N starts without the option information. -k kills the pane when the mode is exited.

    The customize-mode command works only if at least one client is attached.

switch-mode [-kswZ] [-F format] [-t target-pane] [command]
Put a pane into switch mode, where a session or window may be chosen interactively from a list. Each session or window is shown on one line and the list is narrowed by typing: the typed text is matched against each item with fuzzy matching and only matching items are shown, sorted by how well they match. -s lists sessions (the default) and -w lists windows. -Z zooms the pane. The following keys may be used in switch mode:
Key Function
...
Escape Exit mode
    ...
    After a session or window is chosen, the first instance of ‘%%’ and all instances of ‘%1’ are replaced by the target in command and the result executed as a command. If command is not given, "switch-client -Zt '%%'" is used. -F specifies the format for each item in the list. -k kills the pane when the mode is exited.

    The appearance of matched characters is controlled by the switch-mode-match-style option.
...
Like split-window, but instead of splitting dst-pane and creating a new pane, split it and move src-pane into the space. This can be used to reverse break-pane. The -b option causes src-pane to be joined to left of or above dst-pane.
    ...
    If src-pane is floating and dst-pane is either unspecified or equal to src-pane, src-pane is returned to its previous position in the layout.

kill-pane [-a] [-f filter] [-t target-pane]
(alias: killp)
Destroy the given pane. If no panes remain in the containing window, it is also destroyed. The -a option kills all panes in the window except the pane given with -t. When -a is given, -f specifies a filter. Only panes for which the filter is true are killed.
kill-window [-a] [-f filter] [-t target-window]
(alias: killw)
Kill the current window or the window at target-window, removing it from any sessions to which it is linked. The -a option kills all windows in the session except the window given with -t. When -a is given, -f specifies a filter. Only windows for which the filter is true are killed.
...
list-panes [-ars] [-F format] [-f filter] [-O sort-order] [-t target]
(alias: lsp)
If -a is given, target is ignored and all panes on the server are listed. If -s is given, target is a session (or the current session). If neither is given, target is a window (or the current window). -F specifies the format of each line and -f a filter. Only panes for which the filter is true are shown. See the FORMATS section. -O specifies the sort order: one of ‘name’ (title), ‘index’, ‘size’ (area), ‘creation’ (time), or ‘activity’ (time). -r reverses the sort order.
list-windows [-ar] [-F format] [-f filter] [-O sort-order] [-t target-session]
(alias: lsw)
If -a is given, list all windows on the server. Otherwise, list windows in the current session or in target-session. -F specifies the format of each line and -f a filter. Only windows for which the filter is true are shown. See the FORMATS section. -O specifies the sort order: one of ‘index’, ‘name’, ‘size’ (area), ‘creation’ (time), or ‘activity’ (time). -r reverses the sort order.
move-pane [-bdfhMv] [-D [lines]] [-l size] [-L [columns]] [-P position] [-R [columns]] [-s src-pane] [-t dst-pane] [-U [lines]] [-X x-position] [-Y y-position] [-z z-index]
(alias: movep)
Does the same as join-pane, except if given -D, -L, -P, -R, -U, -X, -Y or -z in which case move the target floating pane. -D, -L, -R and -U move it down, left, right or up by lines or columns (one if omitted). -P moves it to position, which may be one of:
Position Meaning
top-left Top left
top-centre Top and horizontal centre
top-right Top right
centre-left Vertical centre and left
centre Centre
centre-right Vertical centre and right
bottom-left Bottom left
bottom-centre Bottom and horizontal centre
bottom-right Bottom right
top-left-centre Centre of top-left quadrant
top-right-centre Centre of top-right quadrant
bottom-left-centre Centre of bottom-left quadrant
bottom-right-centre Centre of bottom-right quadrant
front Front of floating panes
back Back of floating panes
forward Forward one floating pane
backward Backward one floating pane
forward-loop Forward but back if already at front
backward-loop Backward but front if already at back

    -X and -Y move it to an absolute position. -z moves the pane to the given z-index, where zero is the front.
...
new-window [-abdEkPS] [-c start-directory] [-e environment] [-F format] [-n window-name] [-t target-window] [shell-command [argument ...]]
(alias: neww)
Create a new window. With -a or -b, the new window is inserted at the next index after or before the specified target-window, moving windows up if necessary; otherwise target-window is the new window location.
    ...
    The TERM environment variable must be set to ‘screen’ or ‘tmux’ for all programs running inside tmux. New windows will automatically have ‘TERM=screen’ added to their environment, but care must be taken not to reset this in shell startup files or by the -e option.
...
new-pane [-bdefhIkLPvWZ] [-B border-lines] [-c start-directory] [-e environment] [-F format] [-l size] [-m message] [-p percentage] [-R inactive-border-style] [-s style] [-S active-border-style] [-t target-pane] [-T title] [-x width] [-y height] [-X x-position] [-Y y-position] [shell-command [argument ...]]
(alias: newp)
Creates a new floating pane. The -x and -y options set the width and height of the floating pane in columns and lines respectively. The default is half the window width and a quarter the window height. The -X and -Y options set the position of the upper-left corner of the pane. If omitted, new floating panes are cascaded from the top-left of the window. The -x, -y, -X, and -Y options may be followed by ‘%’ to specify a percentage of the window size. -B sets the pane border lines for floating panes; see pane-border-lines.

    The -L option makes new-pane behave like split-window.

    All other options have the same meaning as for the split-window command.
...
resize-pane [-MTZ] [-D lines] [-L columns] [-R columns] [-U lines] [-x width] [-y height] [-t target-pane]
(alias: resizep)
Resize a pane, up, down, left or right by a specified adjustment with -U, -D, -L or -R; or to an absolute size with -x or -y. The adjustment is given in lines or columns (the default is 1); -x and -y may be a given as a number of lines or columns or followed by ‘%’ for a percentage of the window size (for example ‘-x 10%’). If target-pane is floating, -U, -D, -L, and -R target their respective borders, and negative values may be given. With -Z, the active pane is toggled between zoomed (occupying the whole of the window) and unzoomed (its normal position in the layout).
...
respawn-pane [-Ek] [-c start-directory] [-e environment] [-t target-pane] [shell-command [argument ...]]
(alias: respawnp)
Reactivate a pane in which the command has exited (see the remain-on-exit window option). If shell-command is not given, the command used when the pane was created or last respawned is executed. The pane must be already inactive, unless -k is given, in which case any existing command is killed. If -E is given, the pane is left without a running command. -c specifies a new working directory for the pane. The -e option has the same meaning as for the new-window command.
respawn-window [-Ek] [-c start-directory] [-e environment] [-t target-window] [shell-command [argument ...]]
(alias: respawnw)
Reactivate a window in which the command has exited (see the remain-on-exit window option). If shell-command is not given, the command used when the window was created or last respawned is executed. The window must be already inactive, unless -k is given, in which case any existing command is killed. If -E is given, the window is left with one pane and without a running command. -c specifies a new working directory for the window. The -e option has the same meaning as for the new-window command.
...
split-window [-bdefhIkPvZ] [-c start-directory] [-e environment] [-F format] [-l size] [-m message] [-p percentage] [-R inactive-border-style] [-s style] [-S active-border-style] [-t target-pane] [-T title] [shell-command [argument ...]]
(alias: splitw)
Create a new pane by splitting target-pane. If -d is given, the session does not make the new pane the current pane. -Z zooms if the window is not zoomed, or keeps it zoomed if already zoomed. -s sets the style for the pane content. -S sets the border style when the pane is active and -R sets the border style when the pane is inactive (see STYLES). -T sets the pane title.

    -h does a horizontal split and -v a vertical split; if neither is specified, -v is assumed. The -l option specifies the size of the new pane in lines (for vertical split) or in columns (for horizontal split); size may be followed by ‘%’ to specify a percentage of the available space. -p is a shorthand option for this. The -b option causes the new pane to be created to the left of or above target-pane. The -f option creates a new pane spanning the full window height (with -h) or full window width (with -v), instead of splitting the active pane.

    -k keeps the pane open after the optional shell-command exits and waits for a key to be pressed before closing it. The message shown is controlled by the remain-on-exit-format option. -m message is equivalent to -k but also sets the remain-on-exit-format option for this pane to message.

    -W Waits until shell-command exits, then returns its exit status. For example:

    $ tmux new-pane -W 'vi afile'
    $ echo $?
    0

    -E, or an empty shell-command, ('') will create an empty pane with no command running in it; display-message -I can write to an empty pane. The -I flag will create an empty pane and forward any output from stdin to it. For example:
...
(alias: bind)
Bind key key to command. Keys are bound in a key table. By default (without -T), the key is bound in the prefix key table. This table is used for keys pressed after the prefix key (for example, by default ‘c’ is bound to new-window in the prefix table, so ‘C-b c’ creates a new window). The root table is used for keys pressed without the prefix key: binding ‘c’ to new-window in the root table (not recommended) means a plain ‘c’ will create a new window. -n is an alias for -T root. Keys may also be bound in custom key tables and the switch-client -T command used to switch to them from a key binding. The -r flag indicates this key may repeat, see the initial-repeat-time and repeat-time options. -N attaches a note to the key (shown with list-keys -N), which can be cleared by passing an empty string. The -r and -N flags can be used without command to alter an existing binding.

    To view the default bindings and possible commands, see the list-keys command.

list-keys [-1aNr] [-F format] [-O sort-order] [-P prefix-string] [-T key-table] [key]
(alias: lsk)
List key bindings. -F specifies the format of each line. See the FORMATS section. -T specifies a key-table to list from. -1 lists only the first matching key. -O specifies the sort order: one of ‘key’, ‘modifier’, ‘name’ (table name). -r reverses the sort order.

    If no format is given, there are two forms: the default lists keys as bind-key commands; -N lists only keys with attached notes and shows only the key and note for each key.

    With the default form, all key tables are listed unless specified otherwise.

    With the -N form, only keys in the root and prefix key tables are listed by default; -T also lists only keys in key-table. -P specifies a prefix to print before each key. -a lists the command for keys that do not have a note rather than skipping them.
...
The appearance and behaviour of tmux may be modified by changing the value of various options. Each option belongs to one or multiple scopes (server, session, window, and pane) and has a type (string, number, key, colour, flag, choice, or command). Values of flag-type options may be one of: 1, on, yes, 0, off, or no; for possible choice values, see the respective option; for key options, the KEY BINDINGS section; and for colour options, the STYLES section.
...
Set a pane option with -p, a window option with -w, a server option with -s, otherwise a session option. If the option is not a user option, -w or -s may be unnecessary - tmux will infer the scope from the option name, assuming -w for pane options. If -g is given, the global session or window option is set.
    ...
    Array options may be set by giving a key in square brackets after the option name, for example ‘command-alias[zoom]’.
...
This is an array of custom aliases for commands. If an unknown command matches name, it is replaced with value. For example, after:
    ...
    set -s command-alias[zoom] zoom='resize-pane -Z'
...
An array option allowing widths of Unicode codepoints to be overridden. Note the new width applies to all clients. Each entry is of the form codepoint=width, where codepoint may be a UTF-8 character, an identifier of the form ‘U+number’ where the number is a hexadecimal number, or a range of the form ‘U+number-U+number’.
copy-command shell-command
Give the command to pipe to if the copy-pipe copy mode command is used without arguments.
theme [detect | terminal | light | dark]
Whether tmux should use the detected client theme (detect), use terminal ANSI colours (terminal), or force the light or dark theme.
dark-theme-black, dark-theme-white, dark-theme-light-grey, dark-theme-dark-grey, dark-theme-green, dark-theme-yellow, dark-theme-red, dark-theme-blue, dark-theme-cyan, dark-theme-magenta, light-theme-black, light-theme-white, light-theme-light-grey, light-theme-dark-grey, light-theme-green, light-theme-yellow, light-theme-red, light-theme-blue, light-theme-cyan and light-theme-magenta colour
Set the theme colours. These are the named colours themeblack, themewhite, themelightgrey, themedarkgrey, themegreen, themeyellow, themered, themeblue, themecyan and thememagenta which may be used in styles and elsewhere a colour is accepted. The dark-theme-_ set is used when the client reports a dark background and the light-theme-_ set when it reports a light background; an unknown background is treated as dark.
default-client-command command
Set the default command to run when tmux is called without a command. The default is new-session.
...
Controls the behaviour when an application requests the clipboard from tmux.
    ...
    If off, the request is ignored; if buffer, tmux responds with the newest paste buffer; request causes tmux to request the clipboard from the most recently used client (if possible) and send the reply (if any) back to the application; both is the same as request but also creates a paste buffer.
...
Set terminal features for terminal types read from terminfo(5). tmux has a set of named terminal features. Each will apply appropriate changes to the terminfo(5) entry in use.
    ...
    This is an array option where each entry is a colon-separated string made up of a terminal type pattern (matched using glob(7) patterns) followed by a list of terminal features. The available features are:
...
Allow terminal descriptions read using terminfo(5) to be overridden. Each entry is a colon-separated string made up of a terminal type pattern (matched using glob(7) patterns) and a set of name=value entries.
...
display-panes-format format
Set the format of the text shown by the display-panes command, expanded for each pane.
...
Set the time in milliseconds for the initial repeat when a key is bound with the -r flag. This allows multiple commands to be entered without pressing the prefix key again. See also the repeat-time option. If initial-repeat-time is zero, repeat-time is used for the first key press.
key-table key-table
...
Set the format string for the prompt and message area. The special placeholder ‘#{message}’ expands to the interactive prompt or message text and ‘#{command_prompt}’ is set to 1 when the prompt is in command mode (vi). Style directives like fill, align, and width may be used in the format string. The default uses a conditional to select between message-style and message-command-style.
message-line [0 | 1 | 2 | 3 | 4]
Set line on which status line messages and the command prompt are shown.
message-style style
Set status line message style. This is used for messages and for the command prompt. The message is drawn on top of the existing status line. A width attribute (a fixed number of columns or a percentage such as ‘50%’) constrains the prompt area width and an align attribute (left, centre, right) sets its horizontal position. When the width is less than the full terminal width, the normal status bar content remains visible around the prompt area. The fill attribute is used by the default message-format to clear the background. For how to specify style, see the STYLES section.
mouse [on | off]
...
prompt-command-cursor-colour colour
Set the colour of the cursor in the command prompt when vi(1) keys are enabled and the prompt is in command mode.
prompt-cursor-style style
Set the style of the cursor in the command prompt. See the cursor-style options for available styles.
prompt-command-cursor-style style
Set the style of the cursor in the command prompt when vi(1) keys are enabled and the prompt is in command mode. See the cursor-style options for available styles.
...
repeat-time time
Allow multiple commands to be entered without pressing the prefix key again in the specified time milliseconds (the default is 500). Whether a key repeats may be set when it is bound using the -r flag to bind-key. Repeat is enabled for the default keys bound to the resize-pane command. See also the initial-repeat-time option.
set-titles [on | off]
...
copy-mode-current-line-number-style style
Set style of current line number in copy mode. For how to specify style, see the STYLES section.

copy-mode-line-number-style style
Set style of line numbers in copy mode. For how to specify style, see the STYLES section.

copy-mode-line-numbers [off | default | absolute | relative | hybrid]
Show line numbers in copy mode. off hides line numbers, default shows line numbers matching the copy mode position indicator and goto-line, absolute shows absolute line numbers, relative shows line numbers relative to the cursor, and hybrid shows the current line number as absolute and other line numbers as relative. With off or default, the position indicator and goto-line use the same numbering; with absolute, relative and hybrid, they use absolute line numbers.
...
pane-border-status [off | top | bottom | top-floating | bottom-floating]
Turn pane border status lines off or set their position. If set to top-floating or bottom-floating, status lines are shown only on floating panes.
...
pane-scrollbars [off | modal | on | auto-hide]
When enabled, a character based scrollbar appears on the left or right of each pane. A filled section of the scrollbar, known as the ‘slider’, represents the position and size of the visible part of the pane content.
    ...
    If set to on the scrollbar is visible all the time and the pane is narrowed by the width of the scrollbar. If set to modal the scrollbar only appears when the pane is in copy mode or view mode and auto-hides when not in use. If set to auto-hide the scrollbar is available all the time but auto-hides when not in use. With modal and auto-hide, the scrollbar overlays the pane and does not narrow or reflow the pane.
...
pane-scrollbars-timeout time
Set the time in milliseconds before scrollbars are hidden when pane-scrollbars is modal or auto-hide.
...
tree-mode-border-style style
Set the style of borders in tree mode. For how to specify style, see the STYLES section.

tree-mode-preview-format format
Format of the preview indicator in tree mode.

tree-mode-preview-style style
Set the style of the preview indicator in tree mode. For how to specify style, see the STYLES section.

tree-mode-selection-style style
Set the style of the selected line in tree mode. For how to specify style, see the STYLES section.
...
remain-on-exit [on | off | failed | key]
A pane with this flag set is not destroyed when the program running in it exits. If set to failed, then only when the program exit status is not zero. If set to key, the pane stays open and closes when a key is pressed. The pane may be reactivated with the respawn-pane command.
...
switch-mode-match-style style
Set the style of characters matched by the filter in switch-mode. For how to specify style, see the STYLES section.
...
window-pane-status-format string
Set the format in which the window pane is displayed in the status line window pane list. See the FORMATS and STYLES sections.

window-pane-current-status-format string
Like window-status-format, but is the format used when the window is the current window.
...
set-hook -g pane-mode-changed[style] 'display-message changed'
set-option -g pane-mode-changed[style] 'display-message changed'

Setting a hook without specifying an array key clears the hook and sets the first member of the array.
...
Run when an OSC 133 command finishes in a pane. The event payload includes ‘hook_command_start_time’, ‘hook_command_end_time’, ‘hook_command_duration’ and ‘hook_command_status’ if present.
...
set-hook [-agpERTuw] [-B name:what:format] [-t target-pane] hook-name [command]
Without -E or -R, sets (or with -u unsets) hook hook-name to command. The flags are the same as for set-option.

    With -B, name:what:format uses the same subscription syntax as refresh-client -B: name is the hook to run, what selects the session, pane, all panes, window, or all windows, and format is expanded once a second. With -T, the hook is run only when format is true. For monitor hooks, name must begin with ‘@’. If command is given, it is stored as the ‘@’ hook command; otherwise only the monitor is created or replaced. Note that monitor hooks are not inherited, the hook is only run from the scope where it is created. With -u, the subscription named by -B is removed.

    With -E, fire the user event named hook-name. hook-name must begin with ‘@’.

    With -R, run hook-name immediately.

show-hooks [-Bgpw] [-t target-pane] [hook]
Shows hooks. The flags are the same as for show-options.

    With -B, shows the subscriptions installed with set-hook -B.
...
ControlN on control range N

See the STYLES section for information on control ranges.
...
An ‘m’ specifies a glob(7) pattern or regular expression comparison. The first argument is the pattern and the second the string to compare. An optional argument specifies flags: ‘r’ means the pattern is a regular expression instead of the default glob(7) pattern; ‘i’ means to ignore case; ‘z’ means to do a fuzzy match; ‘p’ is like ‘z’ but returns a list of matched positions. A fuzzy match matches plain terms as sequences where each character must appear in order but not necessarily consecutively; terms beginning with ‘'’ are exact substring matches, ‘^’ anchors a term at the start, ‘$’ anchors it at the end, ‘’! inverts a term and ‘|’ separates alternative groups. For example: ‘#{m:_foo_,#{host}}’ or ‘#{m/ri:^A,MYVAR}’ or ‘#{m/z:dev bash,dev:1 bash}’ or ‘#{m/z:^dev | ^prod,prod:1 ssh}’.

A ‘C’ performs a search for a glob(7) pattern or regular expression in the pane content and evaluates to zero if not found, or a line number if found. Like ‘m’, an ‘r’ flag means search for a regular expression and ‘i’ ignores case. For example: ‘#{C/r:^Start}’

Numeric operators may be performed by prefixing two comma-separated alternatives with an ‘e’ and an operator. An optional ‘f’ flag may be given after the operator to use floating point numbers, otherwise integers are used. This may be followed by a number giving the number of decimal places to use for the result. The available operators are: addition ‘+’, subtraction ‘-’, multiplication ‘_’, division ‘/’, modulus ‘m’ or ‘%’ (note that ‘%’ must be escaped as ‘%%’ in formats which are also expanded by strftime(3)) and numeric comparison operators ‘==’, ‘!=’, ‘<’, ‘<=’, ‘>’ and ‘>=’. For example, ‘#{e|_|f|4:5.5,3}’ multiplies 5.5 by 3 for a result with four decimal places and ‘#{e|%%:7,3}’ returns the modulus of 7 and 3. ‘a’ replaces a numeric argument by its ASCII equivalent, so ‘#{a:98}’ results in ‘b’. ‘c’ replaces a tmux colour by its six-digit hexadecimal RGB value. If an argument of ‘f’ or ‘b’ is given, it will instead produce the SGR escape sequence to set the foreground or background colour respectively.
...
Prefixing a time variable with ‘t:’ will convert it to a string, so if ‘#{window_activity}’ gives ‘1445765102’, ‘#{t:window_activity}’ gives ‘Sun Oct 25 09:25:02 2015’. Adding ‘p’ (‘t/p’) will use shorter but less accurate time format for times in the past. ‘r’ (‘t/r’) will give the time relative to the current time, for example ‘1m’ or ‘2m23s’. ‘d’ (‘t/d’) will give the difference from the time to current time in seconds (future times are negative). A custom format may be given using an ‘f’ suffix (note that ‘%’ must be escaped as ‘%%’ if the format is separately being passed through strftime(3), for example in the status-left option): ‘#{t/f/%%H#:%%M:window_activity}’, see strftime(3).

The ‘I:’ prefix will interrogate the client. ‘I/f’ will be true if tmux believes that the client supports the given terminal feature, for example ‘I/f:RGB’, ‘I/c’ will be true if a client has the given terminfo(3) capability, for example ‘I/c:smcup’, and ‘I/e’ gives the value of a client environment variable, for example ‘#{I/e:FOO}’.

The ‘b:’ and ‘d:’ prefixes are basename(3) and dirname(3) of the variable respectively. ‘q:’ will escape sh(1) special characters; with a ‘h’ suffix, escape hash characters (so ‘#’ becomes ‘##’); or with ‘a’ escape tmux command arguments; with ‘s’ use single quotes. ‘E:’ will expand the format twice, for example ‘#{E:status-left}’ is the result of expanding the content of the status-left option rather than the option itself. ‘T:’ is like ‘E:’ but also expands strftime(3) specifiers. ‘S:’, ‘W:’, ‘P:’ or ‘L:’ will loop over each session, window, pane or client and insert the format once for each. ‘L:’, ‘S:’ and ‘W:’ can take an optional sort argument ‘/i’, ‘/n’, ‘/t’ to sort by index, name, or last activity time; additionally ‘/r’ to sort in reverse order. ‘/r’ can also be used with ‘P:’ to reverse the sort order; by default panes are sorted by creation order. ‘P:’ can also take ‘/i’ to sort by pane index or ‘/z’ to sort by z-index. For example, ‘S/nr:’ to sort sessions by name in reverse order. For each, two comma-separated formats may be given: the second is used for the current window, active pane, or active session. For example, to get a list of windows formatted like the status line:

#{W:#{E:window-status-format} ,#{E:window-status-current-format} }

Within the ‘W:’ loop, user options (‘@name’) from neighboring windows are available with a ‘next*’ or ‘prev*’ prefix, for example a user option ‘@color’ on the next window is available as ‘next\_@color’.

‘O:’ will loop over each option; array options are looped once for each array item. ‘O:’ may be given a flag to choose the options table:
Flag Table
s session
w window
p pane
...
‘g’ chooses global options. The default is ‘s’. ‘V:’ will loop over each environment variable. Its flags are:
Flag Environment
...
Multiple modifiers may be separated with a semicolon (;) as in ‘#{T;=10:status-left}’, which limits the resulting strftime(3) -expanded string to at most 10 characters.
...
bracket_paste_flag Pane bracketed paste flag
...
client_colours Number of colours client supports
...
copy_position Position shown in the copy mode indicator
copy_position_limit Limit shown in the copy mode indicator
...
hook_last Previous value for a monitor hook
hook_pane ID of pane where hook was run, if any
hook_session ID of session where hook was run, if any
hook_session_name Name of session where hook was run, if any
hook_value New value for a monitor hook
hook_window ID of window where hook was run, if any
hook_window_index Index of window where hook was run, if any
hook_window_name Name of window where hook was run, if any
host #H Hostname of local host
host_short #h Hostname of local host (no domain name)
insert_flag Pane insert flag
key_string String representation of the key binding
key_repeat 1 if key binding is repeatable
key_note Note of the key binding
key_prefix Global key prefix
key_table Table name of the key binding
key_command Command of the key binding
key_has_repeat 1 if list contain a repeatable key
key_string_width Maximum key_string width in list
key_table_width Maximum key_table width in list
keypad_cursor_flag Pane keypad cursor flag
keypad_flag Pane keypad flag
last_window_index Index of last window in session
line Line number in the list
loop_index Index of item in the W:, P:, S:, or L: loop
...
pane_command_duration Current or most recent OSC 133 command duration in seconds
pane_command_end_time Time most recent OSC 133 command ended
pane_command_running 1 if an OSC 133 command is running
pane_command_start_time Time current or most recent OSC 133 command started
pane_command_status Exit status from most recent OSC 133 command
...
pane_last_output_time Time pane last produced output
pane_last_prompt_time Time most recent OSC 133 prompt began
...
pane_start_command_list Command pane started with, quoted
...
synchronized_output_flag 1 if pane has synchronized output enabled
uid Server UID
user Server user
...
window_manual_height Manual height of window, if set
window_manual_width Manual width of window, if set
...
dim=percentage
Dim foreground and background colours by percentage.
width=N
Set the width of the styled area. N may be a column count or a percentage (for example ‘50%’).
link=uri (or nolink)
Make the styled text an OSC 8 hyperlink to uri, for example ‘#[link=https://example.com]text#[nolink]’. This is emitted only to terminals with the hyperlinks feature (see terminal-features); it works in the status line and in other formats drawn with styles. The link continues until nolink, default, or an empty link= is given. The uri may not contain spaces or commas and is limited in length.
...
range=left, range=right, range=session|X, range=window|X, range=pane|X, range=user|X, range=control|N, norange
Mark a range for mouse events in the status-format option. When a mouse event occurs in the range=left or range=right range, the ‘StatusLeft’ and ‘StatusRight’ key bindings are triggered.
    ...
    range=control|N is a set of ranges for users to define custom behavior. ‘N’ can be 0 through 9. When a mouse event occurs in the range=control|N range, the ‘ControlN’ key binding is triggered. See MOUSE SUPPORT for details.
...
command-prompt [-1bCeFiklNP] [-I inputs] [-p prompts] [-t target-client] [-T prompt-type] [template]
Open the command prompt in a client. This may be used from inside tmux to execute commands interactively.
    ...
    -1 makes the prompt only accept one key press, in this case the resulting input is a single character. -k is like -1 but the key press is translated to a key name. -N makes the prompt only accept numeric key presses. If -C given, panes will continue to be updated while the prompt is displayed. -i executes the command every time the prompt input changes instead of when the user exits the command prompt. -e makes BSpace cancel an empty prompt. -P opens a prompt inside a pane instead of on the status line.
...
display-menu [-OM] [-b border-lines] [-c target-client] [-C starting-choice] [-H selected-style] [-s style] [-S border-style] [-t target-pane] [-T title] [-x position] [-y position] name key command [name key command ...]
(alias: menu)
Display a menu on target-client. target-pane gives the target for any commands run from the menu.
    ...
    L 	Both 	The position of the last menu
    ...
    popup_last_x 	Left of the last menu
    popup_last_y 	Bottom of the last menu
...
(alias: popup)
Display a popup running shell-command (or default-command when omitted) on target-client. A popup is a rectangular box drawn over the top of any panes. If the command is run inside an existing popup, that popup is modified. Only the -b, -B, -C, -E, -EE, -K, -N, -s, and -S options are accepted in this case; all others are ignored.

    -E closes the popup automatically when shell-command exits. Two -E closes the popup only if shell-command exited with success. -k allows any key to dismiss the popup instead of only ‘Escape’ or ‘C-c’.
...
choose-buffer [-kNryZ] [-F format] [-f filter] [-K key-format] [-O sort-order] [-t target-pane] [template]
...
q Exit mode
    ...
    -O specifies the initial sort order: one of ‘creation’ (time), ‘name’, or ‘size’. -r reverses the sort order. -f specifies an initial filter: the filter is a format - if it evaluates to zero, the item in the list is not shown, otherwise it is shown. If a filter would lead to an empty list, it is ignored. -F specifies the format for each item in the list and -K a format for each shortcut key; both are evaluated once for each line. -N starts without the preview. -k kills the pane when the mode is exited. The choose-buffer command works only if at least one client is attached.
...
(alias: lsb)
List the global buffers. -F specifies the format of each line and -f a filter. Only buffers for which the filter is true are shown. See the FORMATS section. -O specifies the sort order: one of ‘name’, ‘size’, or ‘creation’ (time). -r reverses the sort order.
...
paste-buffer [-dprS] [-b buffer-name] [-s separator] [-t target-pane]
(alias: pasteb)
Insert the contents of a paste buffer into the specified pane. If not specified, paste into the current one. By default, control characters are sanitized with vis(3); -S disables this. With -d, also delete the paste buffer.

    When output, any linefeed (LF) characters in the paste buffer are replaced with a separator, by default carriage return (CR). A custom separator may be specified using the -s flag. The -r flag means to do no replacement (equivalent to a separator of LF). If -p is specified, paste bracket control codes are inserted around the buffer if the application has requested bracketed paste mode.
...
(alias: run)
Execute shell-command using /bin/sh or (with -C) a tmux command in the background without creating a window. Before being executed, shell-command is expanded using the rules specified in the FORMATS section. If argument values are given, they are available as ‘#{1}’, ‘#{2}’ and so on. For example:

    run-shell 'myscript.sh #1 #2' foo bar

    With -b, the command is run in the background. -d waits for delay seconds before starting the command. -E redirects the command's stderr to stdout instead of ignoring it. If -c is given, the current working directory is set to start-directory. If -C is not given, any output to stdout is displayed in view mode (in the pane specified by -t or the current pane if omitted) after the command finishes. If the command fails, the exit status is also displayed.
...
When used without options, prevents the client from exiting until woken using wait-for -S with the same channel. When -L is used, the channel is locked and any clients that try to lock the same channel are made to wait until the channel is unlocked with wait-for -U.
    ...
    With -E, tmux waits for the next event with name. Events include hook and notification names, and user ‘@’ events generated by set-hook -E or set-hook -B. If -F is given, format must also be true. If -v is given, event payload keys are printed (whether or not format is true).

    -l list the waiters for name and -w wakes waiter on name immediately.
...
Setulc, Setulc1, ol
Set the underscore colour or reset to the default. Setulc is for RGB colours and Setulc1 for ANSI or 256 colours. The Setulc argument is (red _ 65536) + (green _ 256) + blue where each is between 0 and 255.
Ss, Se
...
%unlinked-window-renamed window-id new-name
