...
Singleton {
    ...
    readonly property string cacheDir: Config.options.launcher ? Config.options.launcher.cacheDir + "/fzf-source" : ""
    ...
    onCacheDirChanged: if (cacheDir.length > 0) Quickshell.execDetached(["mkdir", "-p", root.cacheDir]);
    ...
    function activate(item) {
        ...
        // Live-query sources: script is re-run with the typed query ({q}); no
        // client-side filtering. Wait for the user to type.
        if (item.liveQuery) {
            let slug = String(item.label || item.source).replace(/[^a-zA-Z0-9]/g, "_").toLowerCase().substring(0, 80);
            root.activeCacheFile = root.cacheDir + "/live-" + slug + ".txt";
            root.ready = true;
            root.loading = false;
            return;
        }
        ...
        if (cacheTTL === 0) {
            // Always fresh — use a per-activation file in the cache dir
            root.activeCacheFile = root.cacheDir + "/live-" + Date.now() + ".txt";
            root.runSource();
            return;
        }
    ...
    // --- Live-query source: re-run script with the typed query, parse stdout.
    // No debounce, no cache file — keeps it light.
    function runLiveSource(query) {
        if (!root.activeItem || !query) { Logger.log("[Live] skip: activeItem=" + (!!root.activeItem) + " q=" + query); return; }
        root.loading = true;
        liveSourceProcess.running = false;
        liveSourceProcess._query = query;
        const q = query.replace(/'/g, "'\\''");
        const cmd = root.activeItem.source.replace(/\{q\}/g, "'" + q + "'");
        Logger.log("[Live] run: " + cmd);
        liveSourceProcess.command = ["bash", "-c", cmd];
        liveSourceProcess.running = true;
    }

    Process {
        id: liveSourceProcess
        property string _query: ""
        running: false
        stdout: StdioCollector {
            id: liveCollector
            onStreamFinished: {
                Logger.log("[Live] done q=" + liveSourceProcess._query + " search=" + root.searchText + " bytes=" + liveCollector.text.length);
                if (liveSourceProcess._query !== root.searchText) return;
                root.loading = false;
                root.results = root.parseResults(liveCollector.text);
                Logger.log("[Live] results=" + root.results.length);
                root.batchPreload();
            }
        }
    }
    ...
    onSearchTextChanged: {
        ...
        if (root.activeItem && root.activeItem.liveQuery) {
            if (!searchText) {
                liveSourceProcess.running = false;
                root.results = [];
