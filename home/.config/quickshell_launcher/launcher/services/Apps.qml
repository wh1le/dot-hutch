...
Singleton {
    ...
    function runFzf(query) {
        ...
        fzfProcess.command = ["bash", "-c", "fzf -f '" + qEscaped + "' --tiebreak=begin,chunk < '" + _cacheFile + "'"];
    ...
    function launch(entry) {
        ...
        for (let i = 0; i < entry.command.length; i++) {
            const tok = entry.command[i];
            // Drop desktop field codes (Quickshell encodes %f/%u/%F/%U as @@/@@u/...)
            if (tok === "@@" || tok.indexOf("@@") === 0)
                continue;
            parts.push(tok);
        }
        // Strip a now-dangling "--" separator left by removed field codes
        ...
        let script = Quickshell.env("HOME") + "/.local/bin/public/system-launch-app";
