typeset -A __WH1LE
__WH1LE[BASE16_CONFIG]=~/.config/.base16
...
color() {
  ...
  local BASE16_CONFIG_PREVIOUS="${__WH1LE[BASE16_CONFIG]}.previous"
  local STATUS=0
  ...
  __color() {
    ...
    if [[ -e "$FILE" ]]; then
      ...
      if [ -e "$__WH1LE[BASE16_CONFIG]" ]; then
        cp "$__WH1LE[BASE16_CONFIG]" "$BASE16_CONFIG_PREVIOUS"
      fi

      echo "$SCHEME" >! "$__WH1LE[BASE16_CONFIG]"
      echo "$BACKGROUND" >> "$__WH1LE[BASE16_CONFIG]"
      sh "$FILE"
  ...
  if [ $# -eq 0 ]; then
    if [ -s "$__WH1LE[BASE16_CONFIG]" ]; then
      cat "$__WH1LE[BASE16_CONFIG]"
      local SCHEME=$(head -1 "$__WH1LE[BASE16_CONFIG]")
      __color "$SCHEME"
      return
    else
      SCHEME=help
...
function () {
  ...
  if [[ -s "$__WH1LE[BASE16_CONFIG]" ]]; then
    local SCHEME=$(head -1 "$__WH1LE[BASE16_CONFIG]")
    local BACKGROUND=$(sed -n -e '2 p' "$__WH1LE[BASE16_CONFIG]")
    if [ "$BACKGROUND" != 'dark' -a "$BACKGROUND" != 'light' ]; then
      echo "warning: unknown background type in $__WH1LE[BASE16_CONFIG]"
    fi
    # prevent different background color in tmux
    [ -z "$TMUX" ] && color $SCHEME;
  else
    # Default.
    color default-dark
