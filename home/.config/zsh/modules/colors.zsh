...
color() {
  ...
  __color() {
    ...
    if [[ -e "$FILE" ]]; then
      ...
      if [ -e "$__NIKITA[BASE16_CONFIG]" ]; then
        cp "$__NIKITA[BASE16_CONFIG]" "$BASE16_CONFIG_PREVIOUS"
      fi

      echo "$SCHEME" >! "$__NIKITA[BASE16_CONFIG]"
      echo "$BACKGROUND" >> "$__NIKITA[BASE16_CONFIG]"
      sh "$FILE"
  ...
  if [ $# -eq 0 ]; then
    if [ -s "$__NIKITA[BASE16_CONFIG]" ]; then
      cat "$__NIKITA[BASE16_CONFIG]"
      local SCHEME=$(head -1 "$__NIKITA[BASE16_CONFIG]")
      __color "$SCHEME"
      return
    else
      SCHEME=help
...
function () {
  ...
  if [[ -s "$__NIKITA[BASE16_CONFIG]" ]]; then
    local SCHEME=$(head -1 "$__NIKITA[BASE16_CONFIG]")
    local BACKGROUND=$(sed -n -e '2 p' "$__NIKITA[BASE16_CONFIG]")
    if [ "$BACKGROUND" != 'dark' -a "$BACKGROUND" != 'light' ]; then
      echo "warning: unknown background type in $__NIKITA[BASE16_CONFIG]"
    fi
