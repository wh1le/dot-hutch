...
function _quick_jump_dirs() {
  ...
  for mount in /mnt/*(N); do
    ...
    while IFS= read -r dir; do
      ...
      [[ -d $dir ]] || continue
      print -r -- "$dir"
      ((depth > 0)) && fd -HL -a --type d --max-depth "$depth" -E .git -E .direnv -E __pycache__ . "$dir"
    done <"$file"
  done | sort
}
...
function _wh1le_list_dirs() {
  fd -LH --type d --max-depth 10 --strip-cwd-prefix $FZF_EXCLUDES
}

# two walks: [current dir, shadow walk all roots]
function _wh1le_list_files() {
  fd -LH --type f --max-depth 10 --strip-cwd-prefix $FZF_EXCLUDES
