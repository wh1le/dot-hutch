...
in {
  ...
  fonts = {
    ...
    fontconfig = {
      ...
      defaultFonts = {
        ...
        monospace = [ "Atkinson Hyperlegible Mono" "JetBrainsMono Nerd Font" ];
  ...
  environment.systemPackages = with pkgs; [
    ...
    (python3.withPackages (ps: [ ps.pynvim ps.debugpy ]))
  ];
  ...
  systemd.services.dotfiles-setup = {
    ...
    serviceConfig = {
      ...
      ReadWritePaths = [ "/home/${mainUser}/dot" "/tmp" ];
    };
    script = ''
      set -euo pipefail
      export PATH=${
